...
namespace AdvancedAI.Lists
{
    public static class BossList
    {
        ...
        public static void Init()
        {
            ...
            Logging.Write("BossList: for {0}", StyxWoW.Me.CurrentMap.Name);
            //Logger.WriteFile("BossList: for {0}", StyxWoW.Me.CurrentMap.Name); // + " - " + StyxWoW.Me.CurrentMap.MapDescription);
            foreach (var s in Lists.BossList.CurrentMapBosses)
            {
                //Logger.WriteFile("- boss {0}", s);
                Logging.Write("- boss {0}", s);
            }
            Logging.Write("BossList: contains {0} entries", Lists.BossList.CurrentMapBosses.Count());
