            ...
            Composite kitingBehavior =
                        ...
                        new PrioritySelector(
                            ...
                            new Decorator(ret => bstate == State.Slow,
                                new PrioritySelector(
                                    ...
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("BP: transition from SlowAttack to Moving");
                ...
                //  .. a minimum wait of 250 ms after movement has started in the correct direction
                                    ...
                                    new Action(ret =>
                                    {
                                        if (runawayAttack != null)
                                        {
                                            ...
                                            Logging.WriteDiagnostic("BP: transition from Moving to NonFaceAttack");
                                    ...
                                    new Action(ret =>
                                    {
                                        if (jumpturnAttack != null)
                                        {
                                            if (JumpTurn.IsJumpTurnInProgress())
                                            {
                                                ...
                                                Logging.WriteDiagnostic("BP: transition error - active jumpturn? forcing state JumpTurn");
                                            ...
                                            if (Me.IsMoving && Me.IsSafelyFacing(Me.CurrentTarget, 120f))
                                            {
                                                ...
                                                Logging.WriteDiagnostic("BP: already facing so transition from Moving to AttackNoJumpTurn");
                                            ...
                                            {
                                                bstate = State.JumpTurnAndAttack;
                                                Logging.WriteDiagnostic("BP: transition from Moving to JumpTurn");
                ...
                */
                            ...
                            new Decorator(ret => bstate == State.NonFaceAttack,
                                new PrioritySelector(
                                    new Sequence(
                                        new Action(ret => Logging.WriteDiagnostic("BP: entering NonFaceAttack behavior")),
                                            //Logger.WriteDebug(Color.Cyan, "BP: entering NonFaceAttack behavior")),
                                        runawayAttack ?? new Action(r => { return RunStatus.Failure; }),
                                        new Action(r => { return RunStatus.Failure; })
                                        ),
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("BP: transition from NonFaceAttack to Moving");
                            ...
                            new Decorator(ret => bstate == State.AttackWithoutJumpTurn,
                                new PrioritySelector(
                                    new Sequence(
                                        new Action(ret => Logging.WriteDiagnostic("BP: entering AttackNoJumpTurn behavior")),
                                            //Logger.WriteDebug(Color.Cyan, "BP: entering AttackNoJumpTurn behavior")),
                                        jumpturnAttack ?? new Action(r => { return RunStatus.Failure; }),
                                        new Action(r => { return RunStatus.Failure; })
                                        ),
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("BP: transition from AttackNoJumpTurn to Moving");
                            ...
                            new Decorator(ret => bstate == State.JumpTurnAndAttack,
                                new PrioritySelector(
                                    ...
                                    new Decorator(ret => !JumpTurn.IsJumpTurnInProgress(),
                                        ...
                                        {
                                            bstate = State.Moving;
                                            Logging.WriteDiagnostic("BP: transition from JumpTurn to Moving");
                            ...
                            new Action(ret => Logging.WriteDiagnostic("BP: fell through with state {0}", bstate.ToString()))
                                //Logger.WriteDebug(Color.Cyan, "BP: fell through with state {0}", bstate.ToString()))
                            )
                        )
                    );
        ...
        public static Composite CreateBehavior(Composite jumpturnAttack)
        {
            return
                ...
                // add sanity checks here to test if jump should continue
                            ...
                            new Decorator(ret => jstate == State.Jump,
                                new Sequence(
                                    ...
                                    {
                                        // jump up
                                        Logging.WriteDiagnostic("JT: enter Jump state");
                                    ...
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("JT: transition from Jump to Face");
                            ...
                            new Decorator(ret => jstate == State.Face,
                                new Sequence(
                                    ...
                                    new PrioritySelector(
                                        ...
                                        new Action(ret => Logging.WriteDiagnostic("JT: timed out waiting for SafelyFacing to occur"))
                                            //Logger.WriteDebug("JT: timed out waiting for SafelyFacing to occur"))
                                        ),
                                    new Action(ret =>
                                    {
                                        WoWMovement.StopFace();
                                        ...
                                        {
                                            Logging.WriteDiagnostic("JT: transition from Face to Attack");
                                            //Logger.WriteDebug(Color.Cyan, "JT: transition from Face to Attack");
                                            jstate = State.Attack;
                                        }
                                        else
                                        {
                                            Logging.WriteDiagnostic("JT: transition from Face to FaceRestore");
                            ...
                            new Decorator(ret => jstate == State.Attack,
                                new PrioritySelector(
                                    ...
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("JT: transition from Attack to FaceRestore");
                ...
                */
                                    ...
                                    new Action(ret =>
                                    {
                                        Logging.WriteDiagnostic("JT: transition from FaceRestore to EndJump");
                            ...
                            new Action(ret => Logging.WriteDiagnostic("JT: fell through with state {0}", jstate.ToString()))
                                //Logger.WriteDebug(Color.Cyan, "JT: fell through with state {0}", jstate.ToString()))
                            )
                        ),
                    new Decorator(
                        ...
                        new Action(ret =>
                        {
                            ...
                            jstate = State.Jump;
                            Logging.WriteDiagnostic("Jump Turn");
        ...
        public WoWPoint FindLocation(WoWPoint ptOrigin)
        {
            ...
            for (int arcIndex = 0; arcIndex < RaysToCheck; arcIndex++)
            {
                ...
                for (float distFromOrigin = MinScanDistance; distFromOrigin <= MaxScanDistance; distFromOrigin += IncrementScanDistance)
                {
                    ...
                    Logging.WriteDiagnostic("SafeArea: Found mob-free location ({0:F1} yd radius) at degrees={1:F1} dist={2:F1} on point check# {3} at {4}, {5}, {6}", MinSafeDistance, WoWMathHelper.RadiansToDegrees(checkFacing), distFromOrigin, countPointsChecked, ptDestination.X, ptDestination.Y, ptDestination.Z);
                    Logging.WriteDiagnostic("SafeArea: processing took {0:F0} ms", (DateTime.Now - startFind).TotalMilliseconds);
            ...
            Logging.WriteDiagnostic("SafeArea: No mob-free location ({0:F1} yd radius) found within {1:F1} yds ({2} checked, {3} nav, {4} not safe, {5} range, {6} pt los, {7} mob los)", MinSafeDistance, MaxScanDistance, countPointsChecked, countFailToPointNav, countFailSafe, countFailRange, countFailToPointLoS, countFailToMobLoS);
            ...
            {
                Logging.WriteDiagnostic("SafeArea: choosing best available spot in {0:F1} yd radius where closest mob is {1:F1} yds", MinSafeDistance, Math.Sqrt(furthestNearMobDistSqr));
                Logging.WriteDiagnostic("SafeArea: processing took {0:F0} ms", (DateTime.Now - startFind).TotalMilliseconds);
...
#region Disengage Class
    ...
    public class Disengage
    {
                ...
                new Sequence(
                    new PrioritySelector(
                        new Decorator(
                            ret => slowAttack != null,
                            new Sequence(
                                new Action(r => Logging.WriteDiagnostic("attempting to slow enemies")),
                                    //Logger.WriteDebug("attempting to slow enemies")),
                                slowAttack,
                    ...
                    new Action(r => Logging.WriteDiagnostic("face {0} safespot as needed", dir == Direction.Frontwards ? "towards" : "away from")),
                        //Logger.WriteDebug("face {0} safespot as needed", dir == Direction.Frontwards ? "towards" : "away from")),
                    new Action(ret =>
                    {
                    ...
                    new Action(r => Logging.WriteDiagnostic("wait for facing to complete")),
                        //Logger.WriteDebug("wait for facing to complete")),
                    new PrioritySelector(
                        new Wait(new TimeSpan(0, 0, 1), ret => Me.IsDirectlyFacing(needFacing), new ActionAlwaysSucceed()),
                        new Action(ret =>
                        {
                            Logging.WriteDiagnostic("DIS: timed out waiting to face safe spot - need:{0:F4} have:{1:F4}", needFacing, Me.RenderFacing);
                    ...
                    new Action(ret =>
                    {
                        Logging.WriteDiagnostic("DIS: cancel facing now we point the right way");
                        //Logger.WriteDebug(Color.Cyan, "DIS: cancel facing now we point the right way");
                        WoWMovement.StopFace();
                    }),

                    new Action(r => Logging.WriteDiagnostic("set time of {0} just prior", spell)),
                        //Logger.WriteDebug("set time of {0} just prior", spell)),
                    new Sequence(
                        new PrioritySelector(
                            Spell.BuffSelf(spell),
                            new Action(ret => {
                                Logging.WriteDiagnostic("DIS: {0} cast appears to have failed", spell);
                        ...
                        new Action(ret =>
                        {
                            ...
                            Logging.WriteDiagnostic("DIS: finished {0} cast", spell);
                            Logging.WriteDiagnostic("DIS: jumped {0:F1} yds away from orig={1} to curr={2}", Me.Location.Distance(safeSpot), origSpot, Me.Location);
        ...
        public static bool CanWeDisengage(string spell, Direction dir, int distance)
        {
            ...
            if (safeSpot == WoWPoint.Empty)
            {
                Logging.WriteDiagnostic("DIS: no safe landing spots found for {0}", spell);
                //Logger.WriteDebug(Color.Cyan, "DIS: no safe landing spots found for {0}", spell);
                return false;
            }

            Logging.WriteDiagnostic("DIS: Attempt safe {0} due to {1} @ {2:F1} yds",
                spell,
