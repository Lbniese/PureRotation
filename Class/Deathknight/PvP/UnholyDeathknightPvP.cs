...
namespace AdvancedAI.Spec
{
    class UnholyDeathknightPvP
    {
        ...
        public static string[] Dispels = new[] { "Power Word: Shield", "Dark Soul: Instability", "Dark Soul: Knowledge", "Dark Soul: Misery", "Icy Veins",
        ...
        public static Composite CreateUDKPvPCombat
        {
            get
            {
                return new PrioritySelector(
                    ...
                    Spell.Cast("Death Siphon", ret => StyxWoW.Me.HealthPercent < 50),
                    Spell.Cast("Icebound Fortiude", ret => StyxWoW.Me.HealthPercent < 40),
                    Spell.Cast("Death Strike", ret => StyxWoW.Me.GotTarget && StyxWoW.Me.HealthPercent < 15),
                    // use it to heal with deathcoils.
                    Spell.Cast("Lichborne", ret => (StyxWoW.Me.HealthPercent < 25 && StyxWoW.Me.CurrentRunicPower >= 60)),
                    Spell.Cast("Death Coil", at => Me, ret => StyxWoW.Me.HealthPercent < 50 && StyxWoW.Me.HasAura("Lichborne")),
                    new Throttle(2,
                        new PrioritySelector(
                            Spell.Cast("Blood Tap", ret => Me.HasAura("Blood Charge", 10) && (Me.UnholyRuneCount == 0 || Me.DeathRuneCount == 0 || Me.FrostRuneCount == 0)))),
                    //Dispells
                    new Decorator(ret => Me.CurrentTarget.HasAnyAura(Dispels),
                        new PrioritySelector(
                            Spell.Cast("Icy Touch"))),
                    // AOE
                    //new Decorator (ret => UnfriendlyUnits.Count() >= 2, CreateAoe()),
                    // Execute
                    Spell.Cast("Soul Reaper", ret => StyxWoW.Me.CurrentTarget.HealthPercent < 37),

                    // Diseases
                    Spell.Cast("Outbreak", ret => !StyxWoW.Me.CurrentTarget.HasMyAura("Frost Fever") || !StyxWoW.Me.CurrentTarget.HasMyAura("Blood Plague")),
                    Spell.Cast("Plague Strike", ret => !StyxWoW.Me.CurrentTarget.HasMyAura("Blood Plague") || !StyxWoW.Me.CurrentTarget.HasMyAura("Frost Fever")),
                    ...
                    new Decorator(ret => StyxWoW.Me.CurrentTarget.IsWithinMeleeRange && Me.CurrentTarget.HealthPercent <= 40,
                        new PrioritySelector(
                            Spell.Cast("Unholy Frenzy", ret => StyxWoW.Me.CurrentTarget.IsWithinMeleeRange && !PartyBuff.WeHaveBloodlust),
                    ...
                    //Kill Time
                    Spell.Cast("Dark Transformation", ret => Me.GotAlivePet && !Me.Pet.ActiveAuras.ContainsKey("Dark Transformation") && Me.HasAura("Shadow Infusion", 5)),
                    ...
                    Spell.Cast("Death Coil", ret => (StyxWoW.Me.HasAura(SuddenDoom) || StyxWoW.Me.CurrentRunicPower >= 90)),// && StyxWoW.Me.Auras["Shadow Infusion"].StackCount < 5),
                    Spell.Cast("Scourge Strike"),
                    Spell.Cast("Plague Leech", ret => SpellManager.Spells["Outbreak"].CooldownTimeLeft.Seconds <= 1),
                    ...
                    Spell.Cast("Death Coil", ret => SpellManager.Spells["Lichborne"].CooldownTimeLeft.Seconds >= 4 && Me.CurrentRunicPower < 60 || !Me.HasAura("Conversion")), // || StyxWoW.Me.Auras["Shadow Infusion"].StackCount == 5),
